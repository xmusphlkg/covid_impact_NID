...
datafile_analysis <- datafile_analysis |> 
     group_by(disease_1) |> 
     summarise(Withoutmean = sum(Withoutmean),
               Withoutlower = sum(Withoutlower_95, na.rm = F),
               Withoutupper = sum(Withoutupper_95, na.rm = F),
               Withinmean = sum(Withinmean),
               Withinlower = sum(Withinlower_95, na.rm = F),
               Withinupper = sum(Withinupper_95, na.rm = F)) |> 
     left_join(datafile_class,
               by = c(disease_1 = "disease_list")) |> 
     select(disease_name, class, Withoutmean, Withoutlower, Withoutupper, Withinmean, Withinlower, Withinupper) |> 
     mutate(disease_name = fct_rev(disease_name))

# plot 1 ------------------------------------------------------------------

datafile_forecast <- datafile_analysis |> 
     select(-c(Withinmean, Withinlower, Withinupper)) |> 
     mutate(type = 'Without')
names(datafile_forecast) <- c('disease_name', 'class','mean', 'lower', 'upper', 'type')
datafile_observe <- datafile_analysis |> 
     select(-c(Withoutmean, Withoutlower, Withoutupper)) |> 
     mutate(type = 'Within')
names(datafile_observe) <- c('disease_name', 'class','mean', 'lower', 'upper', 'type')
...
fig1 <- ggplot(data = datafile_plot)+
     ...
     scale_x_discrete()+
     scale_fill_manual(values = c(Within = "#F39B7FFF", Without = '#3C5488FF'))+
...
datafile_plot <- datafile_analysis |> 
     mutate(mean_change = (Withoutmean - Withinmean)/Withoutmean,
