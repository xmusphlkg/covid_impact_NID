...
datafile_analysis <- read.xlsx('./Nation.xlsx', detectDates = T) %>% 
...
auto_select_function <- function(i){
     ...
     fit_goodness <- data.frame(
          ...
          Test = postResample(ts_test_1, outcome_plot_2$mean),
          'Train and Test' = postResample(ts_obse_1, c(outcome_plot_1$fit, outcome_plot_2$mean))
     )
     ...
     fit_goodness <- fit_goodness |> 
          rbind(
               data.frame(
                    ...
                    Test = postResample(ts_test_1, outcome_plot_2$mean),
                    'Train and Test' = postResample(ts_obse_1[-which(is.na(outcome_plot_1$fit))], c(outcome_plot_1$fit[-which(is.na(outcome_plot_1$fit))], outcome_plot_2$mean)))
          )
     ...
     fig1 <- ggplot(data = df_mods)+
          ...
          labs(x = "Date",
               y = 'Cases',
               color = '',
               title = paste0(LETTERS[7], ": Multiple models"))+
     ...
     ggsave(filename = paste0('./outcome/appendix/figure/2_covid_epidemic/', disease_name[i],'.pdf'),
            fig,
