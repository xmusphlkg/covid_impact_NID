...
file_list <- list.files(path = './outcome/appendix/data/0_impact/',
                        full.names = T)
...
datafile_analysis <- datafile_analysis |> 
     mutate(phase = if_else(date > as.Date('2022/11/15'),
                            "Tranmission",
                            "PSHM")) |> 
     group_by(phase, disease_1) |> 
     ...
     select(phase, disease_name, mean, upper, lower, value, class) |> 
...
datafile_observe <- datafile_analysis |> 
     select(phase, disease_name, value, class) |> 
     mutate(upper = NA,
            lower = NA,
            .before = value) |> 
     rename(mean = 'value') |> 
...
write.csv(file = './outcome/appendix/data/0_impact/0_summary_by_disease.csv',
          x = datafile_plot,
          row.names = F)
...
write.csv(file = './outcome/appendix/data/0_impact/0_summary_by_class.csv',
          x = datafile_plot,
          row.names = F)
